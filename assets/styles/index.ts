import { StyleSheet, Dimensions } from "react-native";

export const PRIMARY_COLOR = "#E4B76E";
export const SECONDARY_COLOR = "#AEBFD1";
export const WHITE = "#F6F6F4";
export const GRAY = "#6E6E6E";
export const DARK_GRAY = "#2B2B2B";
export const BLACK = "#2B2B2B";
export const BG_MAIN = "#F6F6F4";
export const BG_GRADIENT = "#F6F6F4";
export const TEXT_PRIMARY = "#2B2B2B";
export const TEXT_SECONDARY = "#6E6E6E";
export const SURFACE_BUTTON = "#F6F6F4";
export const CTA_PRIMARY = "#E4B76E";
export const GLOW_HIGHLIGHT = "rgba(43, 43, 43, 0.06)";
export const BUTTON_CREAM = "#F6F6F4";
export const BUTTON_BORDER = "rgba(43, 43, 43, 0.08)";
export const WARM_SHADOW = "rgba(43, 43, 43, 0.10)";
export const SERIF_FONT = "CormorantGaramond_500Medium";

export const ONLINE_STATUS = "#AEBFD1";
export const OFFLINE_STATUS = "#D88C7A";

export const STAR_ACTIONS = "#E4B76E";
export const LIKE_ACTIONS = "#E4B76E";
export const DISLIKE_ACTIONS = "#D88C7A";
export const FLASH_ACTIONS = "#AEBFD1";

export const DIMENSION_WIDTH = Dimensions.get("window").width;
export const DIMENSION_HEIGHT = Dimensions.get("window").height;

export default StyleSheet.create({
  // COMPONENT - CARD ITEM
  containerCardItem: {
    
    borderRadius: 8,
    alignItems: "center",
    margin: 10,
    elevation: 1,
    
  },
  containerCardItemDiscover: {
    borderRadius: 28,
    alignItems: "center",
    paddingVertical: 26,
    paddingHorizontal: 22,
    marginHorizontal: 0,
    marginTop: 8,
    width: DIMENSION_WIDTH - 20,
    alignSelf: "center",
    minHeight: DIMENSION_HEIGHT * 0.82,
    backgroundColor: "#FBF7F4",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.06)",
    shadowOpacity: 0.1,
    shadowRadius: 14,
    shadowColor: BLACK,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
    overflow: "hidden",
    position: "relative",
  },
  discoverCardBackground: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "#FBF7F4",
    zIndex: 0,
  },
  discoverCardBackgroundImage: {
    borderRadius: 24,
    height: "120%",
    transform: [{ translateY: -4,  }, { scale: 1.05 }],
  },
  matchesCardItem: {
    backgroundColor: SECONDARY_COLOR,
    paddingVertical: 7,
    paddingHorizontal: 20,
    borderRadius: 20,
  },
  matchesTextCardItem: {
    color: WHITE,
  },
  matchesTextCardItemDiscover: {
    color: DARK_GRAY,
  },
  descriptionCardItem: {
    color: GRAY,
    textAlign: "center",
    paddingHorizontal: 10,
  },
  descriptionCardItemDiscover: {
    textAlign: "center",
    paddingHorizontal: 18,
    marginTop: 6,
    fontFamily: SERIF_FONT,
  },
  status: {
    paddingBottom: 10,
    flexDirection: "row",
    alignItems: "center",
  },
  statusText: {
    color: GRAY,
    fontSize: 12,
  },
  online: {
    width: 6,
    height: 6,
    backgroundColor: ONLINE_STATUS,
    borderRadius: 3,
    marginRight: 4,
  },
  offline: {
    width: 6,
    height: 6,
    backgroundColor: OFFLINE_STATUS,
    borderRadius: 3,
    marginRight: 4,
  },
  actionsCardItem: {
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 30,
  },
  actionsCardItemDiscover: {
    paddingVertical: 18,
    justifyContent: "space-between",
  },
  cardImageWrap: {
    position: "relative",
  },
  soulmateAvatarWrap: {
    width: 170,
    height: 170,
    borderRadius: 85,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
  },
  soulmateAvatarHalo: {
    position: "absolute",
    width: 180,
    height: 180,
    borderRadius: 90,
    top: -5,
    left: -5,
    zIndex: 0,
  },
  soulmateAvatarImage: {
    width: 150,
    height: 150,
    borderRadius: 75,
    zIndex: 1,
  },
  discoverHeaderWrap: {
    width: "100%",
    alignItems: "center",
    zIndex: 2,
  },
  discoverScroll: {
    width: "100%",
    flex: 1,
    zIndex: 2,
  },
  discoverScrollContent: {
    paddingBottom: 110,
  },
  discoverTitle: {
    color: TEXT_PRIMARY,
    fontSize: 28,
    lineHeight: 32,
    textAlign: "center",
    fontFamily: "CormorantGaramond_700Bold",
    letterSpacing: 0.4,
  },
  discoverAge: {
    marginTop: 2,
    color: TEXT_SECONDARY,
    fontSize: 16,
    lineHeight: 20,
    textAlign: "center",
    fontFamily: SERIF_FONT,
  },
  discoverSubtitle: {
    marginTop: 8,
    color: TEXT_SECONDARY,
    fontSize: 19,
    lineHeight: 26,
    textAlign: "center",
    fontFamily: SERIF_FONT,
  },
  discoverDivider: {
    marginTop: 18,
    marginBottom: 16,
    width: "56%",
    height: 1,
    backgroundColor: "rgba(43, 43, 43, 0.10)",
    zIndex: 2,
  },
  discoverPhotoCard: {
    width: "100%",
    borderRadius: 28,
    backgroundColor: "rgba(246, 246, 244, 0.82)",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.06)",
    overflow: "hidden",
    zIndex: 2,
  },
  discoverPhotoTouch: {
    width: "100%",
    aspectRatio: 0.82,
    position: "relative",
  },
  discoverPhoto: {
    width: "100%",
    height: "100%",
    borderTopLeftRadius: 28,
    borderTopRightRadius: 28,
  },
  discoverPhotoFade: {
    position: "absolute",
    left: 0,
    right: 0,
    bottom: 0,
    height: "28%",
  },
  matchesCardOverlay: {
    position: "absolute",
    bottom: 12,
    alignSelf: "center",
  },
  discoverMatchesOverlay: {
    position: "absolute",
    top: 12,
    right: 12,
  },
  discoverMatchesPill: {
    backgroundColor: "#F6F6F4",
    paddingVertical: 6,
    paddingHorizontal: 14,
    borderRadius: 16,
    borderWidth: 1,
    borderColor: "#AEBFD1",
  },
  discoverContent: {
    alignItems: "center",
    width: "100%",
    paddingHorizontal: 10,
  },
  discoverName: {
    color: TEXT_PRIMARY,
    fontSize: 22,
    lineHeight: 28,
    textAlign: "center",
    fontFamily: "CormorantGaramond_600SemiBold",
    marginTop: 0,
    marginBottom: 0,
  },
  discoverLocation: {
    color: TEXT_SECONDARY,
    fontSize: 14,
    marginBottom: 2,
    fontFamily: SERIF_FONT,
  },
  discoverDescription: {
    textAlign: "left",
    paddingHorizontal: 0,
    marginTop: 4,
    color: "#1F1B16",
    fontSize: 16,
    lineHeight: 24,
    fontFamily: SERIF_FONT,
  },
  discoverInfoSection: {
    marginTop: 20,
    width: "100%",
    zIndex: 2,
  },
  discoverSectionTitle: {
    color: "#17130F",
    fontSize: 17,
    lineHeight: 20,
    fontFamily: "CormorantGaramond_600SemiBold",
    marginBottom: 4,
    flexShrink: 1,
    flexWrap: 'nowrap',
    textAlign: 'left',
    alignSelf: 'flex-start',
    maxWidth: '100%',
    includeFontPadding: false,
  },
  discoverTagRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    justifyContent: "center",
    marginTop: 8,
  },
  discoverTagRowLeft: {
    flexDirection: "row",
    flexWrap: "wrap",
    justifyContent: "flex-start",
    marginTop: 2,
  },
  discoverTagPill: {
    backgroundColor: SURFACE_BUTTON,
    paddingVertical: 6,
    paddingHorizontal: 12,
    borderRadius: 22,
    marginRight: 8,
    marginBottom: 8,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
  },
  discoverTagText: {
    color: "#1F1B16",
    fontSize: 15,
    fontFamily: SERIF_FONT,
  },
  discoverPreferencesSectionTitle: {
    color: "#5D7FA3",
  },
  discoverPreferencePill: {
    backgroundColor: "rgba(174, 191, 209, 0.16)",
    borderColor: "rgba(174, 191, 209, 0.55)",
  },
  discoverPreferenceTagText: {
    color: "#4F6F92",
  },
  discoverSharedSection: {
    marginTop: 2,
    gap: 10,
  },
  discoverSharedItem: {
    flexDirection: "row",
    alignItems: "flex-start",
    backgroundColor: "rgba(246, 246, 244, 0.92)",
    borderRadius: 18,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
    paddingHorizontal: 12,
    paddingVertical: 11,
  },
  discoverSharedIconWrap: {
    width: 32,
    height: 32,
    borderRadius: 16,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "rgba(216, 140, 122, 0.12)",
    marginRight: 10,
    marginTop: 1,
  },
  discoverSharedTextWrap: {
    flex: 1,
  },
  discoverSharedLabel: {
    color: TEXT_SECONDARY,
    fontSize: 10,
    lineHeight: 12,
    letterSpacing: 0.3,
    textTransform: "uppercase",
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  discoverSharedValue: {
    marginTop: 2,
    color: "#17130F",
    fontSize: 18,
    lineHeight: 22,
    fontFamily: "CormorantGaramond_500Medium",
  },
  discoverPromptPill: {
    marginTop: 8,
    backgroundColor: "#F6F6F4",
    paddingVertical: 12,
    paddingHorizontal: 18,
    borderRadius: 34,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
    width: "92%",
  },
  discoverPromptText: {
    color: TEXT_PRIMARY,
    fontSize: 14,
    lineHeight: 22,
    textAlign: "center",
    fontFamily: SERIF_FONT,
  },
  discoverPromptPillFull: {
    marginTop: 2,
    backgroundColor: "#F6F6F4",
    paddingVertical: 14,
    paddingHorizontal: 16,
    borderRadius: 20,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
    width: "100%",
  },
  discoverPromptTextLeft: {
    color: "#1F1B16",
    fontSize: 16,
    lineHeight: 24,
    textAlign: "left",
    fontFamily: SERIF_FONT,
  },
  discoverDebugBox: {
    width: "100%",
    borderRadius: 16,
    backgroundColor: "#F1ECE6",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.08)",
    paddingVertical: 12,
    paddingHorizontal: 14,
  },
  discoverDebugText: {
    color: TEXT_PRIMARY,
    fontSize: 12,
    lineHeight: 18,
    fontFamily: "Courier",
  },
  discoverGalleryRow: {
    marginTop: 14,
    flexDirection: "row",
    justifyContent: "center",
    alignItems: "center",
    gap: 10,
    paddingHorizontal: 2,
    zIndex: 2,
  },
  discoverGalleryThumbWrap: {
    borderRadius: 14,
    overflow: "hidden",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.08)",
    backgroundColor: "#F6F6F4",
  },
  discoverGalleryThumbWrapActive: {
    borderColor: PRIMARY_COLOR,
    borderWidth: 2,
  },
  discoverGalleryThumb: {
    width: 78,
    height: 78,
  },
  discoverPrimaryButton: {
    marginTop: 26,
    width: "100%",
    height: 74,
    borderRadius: 37,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "#E5BC95",
    zIndex: 2,
  },
  discoverPrimaryButtonText: {
    color: WHITE,
    fontSize: 22,
    lineHeight: 28,
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  discoverSecondaryAction: {
    marginTop: 12,
    width: "100%",
    alignItems: "center",
    paddingTop: 14,
    borderTopWidth: 1,
    borderTopColor: "rgba(43, 43, 43, 0.08)",
    zIndex: 2,
  },
  discoverSecondaryActionText: {
    color: TEXT_SECONDARY,
    fontSize: 20,
    lineHeight: 26,
    fontFamily: "CormorantGaramond_500Medium",
  },
  discoverConnectButton: {
    width: "100%",
    height: 64,
    borderRadius: 32,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "#E5BC95",
    zIndex: 2,
  },
  discoverFixedFooter: {
    position: "absolute",
    left: 22,
    right: 22,
    bottom: 18,
    zIndex: 3,
  },
  discoverConnectButtonText: {
    color: WHITE,
    fontSize: 22,
    lineHeight: 28,
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  discoverOrbitScreen: {
    flex: 1,
    width: "100%",
  },
  discoverOrbitCanvas: {
    ...StyleSheet.absoluteFillObject,
  },
  discoverOrbitCenter: {
    position: "absolute",
    width: 180,
    height: 180,
    borderRadius: 90,
    alignSelf: "center",
    top: "29%",
    backgroundColor: "rgba(174, 191, 209, 0.12)",
    borderWidth: 1,
    borderColor: "rgba(174, 191, 209, 0.24)",
  },
  discoverCenterProfileWrap: {
    position: "absolute",
    left: "50%",
    top: "29%",
    width: 180,
    height: 180,
    marginLeft: -90,
    marginTop: 0,
    alignItems: "center",
    justifyContent: "center",
  },
  discoverCenterProfileRing: {
    position: "absolute",
    width: 136,
    height: 136,
    borderRadius: 68,
    borderWidth: 1.5,
    borderColor: "rgba(174, 191, 209, 0.48)",
    borderStyle: "dashed",
  },
  discoverCenterProfileRingInner: {
    position: "absolute",
    width: 120,
    height: 120,
    borderRadius: 60,
    borderWidth: 1,
    borderColor: "rgba(229, 188, 149, 0.54)",
    borderStyle: "dashed",
  },
  discoverCenterProfileImageWrap: {
    width: 98,
    height: 98,
    borderRadius: 49,
    padding: 3,
    backgroundColor: "rgba(246, 246, 244, 0.95)",
    borderWidth: 1.5,
    borderColor: "rgba(246, 246, 244, 0.92)",
    shadowColor: BLACK,
    shadowOpacity: 0.18,
    shadowOffset: { width: 0, height: 4 },
    shadowRadius: 10,
    elevation: 4,
  },
  discoverCenterProfileImage: {
    width: "100%",
    height: "100%",
    borderRadius: 999,
  },
  discoverOrbitUserBubble: {
    position: "absolute",
    left: 0,
    top: 0,
  },
  discoverOrbitUserTapTarget: {
    alignItems: "center",
    justifyContent: "center",
  },
  discoverOrbitUserRing: {
    width: "100%",
    height: "100%",
    borderRadius: 999,
    padding: 3,
    borderWidth: 1.5,
    borderColor: "rgba(246, 246, 244, 0.9)",
    backgroundColor: "rgba(251, 247, 244, 0.82)",
    shadowColor: BLACK,
    shadowOpacity: 0.14,
    shadowOffset: { width: 0, height: 3 },
    shadowRadius: 8,
    elevation: 3,
  },
  discoverOrbitUserImage: {
    width: "100%",
    height: "100%",
    borderRadius: 999,
  },
  discoverOrbitDecorativeDotBlue: {
    position: "absolute",
    left: 0,
    top: 0,
    backgroundColor: "rgba(174, 191, 209, 0.72)",
  },
  discoverOrbitDecorativeDotOrange: {
    position: "absolute",
    left: 0,
    top: 0,
    backgroundColor: "rgba(229, 188, 149, 0.78)",
  },
  discoverSheetRoot: {
    flex: 1,
    justifyContent: "flex-end",
  },
  discoverSheetBackdrop: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "rgba(43, 43, 43, 0.34)",
  },
  discoverSheetCloseButton: {
    position: "absolute",
    right: 18,
    bottom: "92%",
    width: 46,
    height: 46,
    borderRadius: 23,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "#F6F6F4",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.10)",
    shadowOpacity: 0.14,
    shadowRadius: 12,
    shadowColor: BLACK,
    shadowOffset: { height: 6, width: 0 },
    elevation: 5,
    zIndex: 4,
  },
  discoverSheetContainer: {
    width: "100%",
    maxHeight: "92%",
    paddingTop: 10,
    paddingBottom: 14,
    borderTopLeftRadius: 26,
    borderTopRightRadius: 26,
    backgroundColor: "#F6F6F4",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.08)",
  },
  discoverSheetHandle: {
    alignSelf: "center",
    width: 72,
    height: 5,
    borderRadius: 3,
    marginBottom: 6,
    backgroundColor: "rgba(43, 43, 43, 0.18)",
  },
  cardThumbRow: {
    flexDirection: "row",
    justifyContent: "center",
    marginTop: 18,
    marginBottom: 10,
  },
  cardThumbWrap: {
    marginHorizontal: 8,
    borderRadius: 18,
    overflow: "hidden",
    borderWidth: 1,
    borderColor: "rgba(228, 183, 110, 0.9)",
    backgroundColor: "rgba(246, 246, 244, 0.6)",
  },
  cardThumb: {
    width: 78,
    height: 78,
    borderRadius: 18,
  },
  contactButton: {
    marginTop: 8,
    marginBottom: 4,
    backgroundColor: SURFACE_BUTTON,
    paddingVertical: 8,
    paddingHorizontal: 14,
    borderRadius: 16,
    flexDirection: "row",
    alignItems: "center",
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
  },
  contactButtonDiscover: {
    alignSelf: "flex-start",
    marginTop: 12,
    marginBottom: 8,
    backgroundColor: CTA_PRIMARY,
    paddingVertical: 8,
    paddingHorizontal: 16,
    borderRadius: 18,
    shadowOpacity: 0.1,
    shadowRadius: 8,
    shadowColor: BLACK,
    shadowOffset: { height: 2, width: 0 },
  },
  contactButtonText: {
    color: TEXT_PRIMARY,
    fontSize: 13,
    fontFamily: SERIF_FONT,
    marginLeft: 6,
  },
  welcomeContainer: {
    flex: 1,
    paddingHorizontal: 20,
    justifyContent: "space-between",
  },
  welcomeTop: {
    paddingTop: 90,
    alignItems: "center",
  },
  welcomeHaloContainer: {
    width: 300,
    height: 300,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
  },
  welcomeHalo1: {
    position: "absolute",
    width: 250,
    height: 250,
    opacity: 0.8,
    zIndex: 1,
  },
  welcomeHalo2: {
    position: "absolute",
    width: 300,
    height: 300,
    opacity: 0.5,
    zIndex: 0,
  },
  welcomeTitle: {
    fontSize: 38,
    color: TEXT_PRIMARY,
    marginBottom: 6,
    fontWeight: "700",
    textShadowColor: "rgba(246, 246, 244, 0.9)",
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 3,
  },
  welcomeSubtitle: {
    fontSize: 14,
    color: TEXT_SECONDARY,
    textAlign: "center",
    maxWidth: 280,
  },
  welcomeCard: {
    backgroundColor: "rgba(246, 246, 244, 0.0)",
    borderRadius: 22,
    paddingVertical: 22,
    paddingHorizontal: 20,
    marginTop: 30,
    alignItems: "center",
    borderWidth: 0,
    shadowOpacity: 0,
  },
  welcomeQuestion: {
    fontSize: 16,
    color: WHITE,
    textAlign: "center",
    marginTop: 6,
  },
  welcomeButtons: {
    marginTop: 22,
    width: "100%",
    gap: 16,
  },
  welcomeLogo: {
    alignContent: "center",
    justifyContent: "center",
    top: 50,

    opacity: 1,
    position: "absolute",
    width: DIMENSION_WIDTH,
    height: 280,
  },

  welcomePrimary: {
    backgroundColor: CTA_PRIMARY,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    marginVertical: 22,
    borderWidth: 1,
    borderColor: "rgba(228, 183, 110, 0.7)",
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  welcomePrimaryText: {
    color: WHITE,
    fontSize: 20,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
  },
  welcomeSecondary: {
    backgroundColor: BUTTON_CREAM,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    shadowOpacity: 0.14,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 4, width: 0 },
    elevation: 2,
    marginBottom: 40,
  },
  welcomeSecondaryText: {
    color: TEXT_PRIMARY,
    fontSize: 20,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
  },
  loginContainer: {
    flex: 1,
    justifyContent: "center",
    paddingHorizontal: 20,
    marginTop: 100,
  },
  loginCard: {
    backgroundColor: "rgba(254, 254, 254, 0.18)",
    borderRadius: 28,
    paddingVertical: 24,
    paddingHorizontal: 20,
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.55)",
    shadowOpacity: 0.16,
    shadowRadius: 22,
    shadowColor: "#8B97CF",
    shadowOffset: { height: 10, width: 0 },
    zIndex: 1,
  },
  loginTitle: {
    fontSize: 22,
    color: "#243D8E",
    fontFamily: "CormorantGaramond_700Bold",
    textAlign: "center",
  },
  loginSubtitle: {
    fontSize: 13,
    color: "#7680BE",
    fontFamily: SERIF_FONT,
    textAlign: "center",
    marginTop: 6,
  },
  loginField: {
    marginTop: 16,
  },
  loginLabel: {
    fontSize: 12,
    color: "#7680BE",
    fontFamily: SERIF_FONT,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginBottom: 8,
  },
  loginInput: {
    backgroundColor: "rgba(254, 254, 254, 0.12)",
    borderRadius: 999,
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.78)",
    paddingVertical: 14,
    paddingHorizontal: 20,
    color: "#243D8E",
    fontSize: 18,
    fontFamily: SERIF_FONT,
    shadowColor: "#FFFFFF",
    shadowOpacity: 0.1,
    shadowRadius: 10,
    shadowOffset: { width: 0, height: 2 },
  },
  loginError: {
    marginTop: 12,
    color: "#E05B4B",
    textAlign: "center",
    fontSize: 12,
    fontFamily: SERIF_FONT,
  },
  loginButton: {
    backgroundColor: CTA_PRIMARY,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    marginTop: 18,
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  loginButtonDisabled: {
    opacity: 0.6,
  },
  loginButtonText: {
    color: WHITE,
    fontSize: 16,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
  },
  loginSecondary: {
    backgroundColor: BUTTON_CREAM,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    marginTop: 12,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    shadowOpacity: 0.14,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 4, width: 0 },
    elevation: 2,
  },
  loginSecondaryText: {
    color: TEXT_PRIMARY,
    fontSize: 16,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
  },
  galleryOverlay: {
    flex: 1,
    backgroundColor: "rgba(43, 43, 43, 0.9)",
  },
  galleryClose: {
    position: "absolute",
    top: 50,
    right: 20,
    zIndex: 10,
    width: 36,
    height: 36,
    borderRadius: 18,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "rgba(43, 43, 43, 0.6)",
  },
  gallerySlide: {
    width: DIMENSION_WIDTH,
    height: DIMENSION_HEIGHT,
    alignItems: "center",
    justifyContent: "center",
  },
  galleryImage: {
    width: DIMENSION_WIDTH,
    height: DIMENSION_HEIGHT * 0.7,
    resizeMode: "contain",
  },
  contactOverlay: {
    flex: 1,
    backgroundColor: "rgba(43, 43, 43, 0.35)",
    justifyContent: "flex-end",
  },
  contactCard: {
    backgroundColor: WHITE,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    padding: 18,
    maxHeight: DIMENSION_HEIGHT * 0.7,
  },
  contactHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 10,
  },
  contactTitle: {
    fontSize: 18,
    color: DARK_GRAY,
  },
  contactSectionTitle: {
    color: PRIMARY_COLOR,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginTop: 10,
    marginBottom: 6,
  },
  contactSectionHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  },
  contactRow: {
    paddingVertical: 6,
    borderBottomWidth: 1,
    borderBottomColor: "#E4B76E",
  },
  contactRowTitle: {
    color: DARK_GRAY,
    fontSize: 14,
  },
  contactRowMeta: {
    color: GRAY,
    fontSize: 12,
    marginTop: 2,
  },
  swipeBadgeLike: {
    position: "absolute",
    top: 160,
    left: 24,
    zIndex: 20,
    paddingVertical: 8,
    paddingHorizontal: 16,
    borderRadius: 6,
    borderWidth: 3,
    borderColor: "#AEBFD1",
    backgroundColor: "rgba(246, 246, 244, 0.9)",
  },
  swipeBadgeTextLike: {
    fontSize: 28,
    fontWeight: "800",
    color: "#AEBFD1",
    letterSpacing: 2,
  },
  swipeBadgeNope: {
    position: "absolute",
    top: 160,
    right: 24,
    zIndex: 20,
    paddingVertical: 8,
    paddingHorizontal: 16,
    borderRadius: 6,
    borderWidth: 3,
    borderColor: "#D88C7A",
    backgroundColor: "rgba(246, 246, 244, 0.9)",
  },
  swipeBadgeTextNope: {
    fontSize: 28,
    fontWeight: "800",
    color: "#D88C7A",
    letterSpacing: 2,
  },
  swipeOverlay: {
    position: "absolute",
    top: 140,
    left: 0,
    right: 0,
    alignItems: "center",
    justifyContent: "center",
    zIndex: 20,
  },
  swipeIconWrap: {
    width: 90,
    height: 90,
    borderRadius: 45,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: WHITE,
    shadowOpacity: 0.18,
    shadowRadius: 16,
    shadowColor: BLACK,
    shadowOffset: { height: 8, width: 0 },
  },
  swipeText: {
    marginTop: 10,
    color: DARK_GRAY,
    fontSize: 14,
    backgroundColor: WHITE,
    paddingHorizontal: 12,
    paddingVertical: 6,
    borderRadius: 14,
    overflow: "hidden",
    textAlign: "center",
  },
  vibeRow: {
    flexDirection: "row",
    marginTop: 6,
    marginBottom: 8,
    flexWrap: "wrap",
  },
  vibePill: {
    backgroundColor: SURFACE_BUTTON,
    paddingVertical: 4,
    paddingHorizontal: 10,
    borderRadius: 14,
    marginRight: 8,
    marginBottom: 6,
  },
  vibeText: {
    color: TEXT_PRIMARY,
    fontSize: 12,
  },
  promptLabel: {
    color: PRIMARY_COLOR,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginTop: 6,
  },
  promptText: {
    color: GRAY,
    textAlign: "center",
    paddingHorizontal: 12,
  },
  promptTextDiscover: {
    textAlign: "center",
    paddingHorizontal: 18,
    fontFamily: SERIF_FONT,
  },
  button: {
    width: 60,
    height: 60,
    borderRadius: 30,
    backgroundColor: BUTTON_CREAM,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    marginHorizontal: 7,
    alignItems: "center",
    justifyContent: "center",
    elevation: 1,
    shadowOpacity: 0.2,
    shadowRadius: 20,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 10, width: 0 },
  },
  miniButton: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: BUTTON_CREAM,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    marginHorizontal: 7,
    alignItems: "center",
    justifyContent: "center",
    elevation: 1,
    shadowOpacity: 0.15,
    shadowRadius: 20,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 10, width: 0 },
  },

  // COMPONENT - CITY
  city: {
    backgroundColor: WHITE,
    padding: 10,
    borderRadius: 20,
    width: 100,
    elevation: 1,
    shadowOpacity: 0.05,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 0, width: 0 },
  },
  cityText: {
    color: DARK_GRAY,
    fontSize: 13,
    textAlign: "center",
  },

  // COMPONENT - FILTERS
  filters: {
    backgroundColor: BUTTON_CREAM,
    paddingVertical: 8,
    paddingHorizontal: 12,
    borderRadius: 24,
    minWidth: 140,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    elevation: 1,
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 5, width: 0 },
  },
  filtersRow: {
    flexDirection: "row",
    alignItems: "center",
  },
  filtersIcon: {
    width: 26,
    height: 26,
    borderRadius: 12,
    backgroundColor: "#E4B76E",
    alignItems: "center",
    justifyContent: "center",
    marginRight: 8,
  },
  filtersLabel: {
    color: GRAY,
    fontSize: 10,
    textTransform: "uppercase",
    letterSpacing: 1,
  },
  filtersValue: {
    color: DARK_GRAY,
    fontSize: 13,
    marginTop: 2,
  },
  filtersChevron: {
    marginLeft: 10,
  },

  // COMPONENT - MESSAGE
  containerMessage: {
    flex: 1,
    alignItems: "center",
    justifyContent: "flex-start",
    flexDirection: "row",
    width: "100%",
  },
  avatar: {
    borderRadius: 30,
    width: 60,
    height: 60,
    marginRight: 20,
    marginVertical: 6,
  },
  message: {
    color: GRAY,
    fontSize: 12,
    paddingTop: 5,
  },
  messageName: {
    color: DARK_GRAY,
    fontSize: 16,
    fontWeight: "600",
  },

  // COMPONENT - PROFILE ITEM
  containerProfileItem: {
    backgroundColor: WHITE,
    paddingHorizontal: 10,
    paddingBottom: 25,
    margin: 20,
    borderRadius: 8,
    marginTop: -65,
    elevation: 1,
    shadowOpacity: 0.08,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 0, width: 0 },
  },
  matchesProfileItem: {
    width: 135,
    marginTop: -15,
    backgroundColor: SECONDARY_COLOR,
    paddingVertical: 7,
    paddingHorizontal: 20,
    borderRadius: 20,
    alignSelf: "center",
  },
  matchesTextProfileItem: {
    color: WHITE,
    textAlign: "center",
  },
  name: {
    paddingTop: 25,
    paddingBottom: 5,
    color: DARK_GRAY,
    fontSize: 15,
    textAlign: "center",
  },
  descriptionProfileItem: {
    color: GRAY,
    textAlign: "center",
    paddingBottom: 20,
    fontSize: 13,
  },
  info: {
    paddingVertical: 8,
    flexDirection: "row",
    alignItems: "flex-start",
  },
  iconProfile: {
    fontSize: 12,
    color: DARK_GRAY,
    paddingHorizontal: 10,
  },
  infoContent: {
    color: GRAY,
    fontSize: 13,
    lineHeight: 18,
    flex: 1,
    flexWrap: "wrap",
  },
  profileSection: {
    marginTop: 12,
    paddingTop: 10,
    borderTopWidth: 1,
    borderTopColor: "#E4B76E",
  },
  profileSectionTitle: {
    color: PRIMARY_COLOR,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginBottom: 6,
  },
  profileRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingVertical: 6,
  },
  profileRowTitle: {
    color: DARK_GRAY,
    fontSize: 14,
  },
  profileRowMeta: {
    color: GRAY,
    fontSize: 12,
    marginTop: 2,
  },
  profileBadge: {
    backgroundColor: "#E4B76E",
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderRadius: 12,
  },
  profileBadgeText: {
    color: DARK_GRAY,
    fontSize: 12,
  },

  // CONTAINER - GENERAL
  bg: {
    flex: 1,
    resizeMode: "cover",
    backgroundColor: BG_MAIN,
  },
  top: {
    paddingTop: 50,
    marginHorizontal: 10,
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  },
  title: { paddingBottom: 10, fontSize: 22, color: TEXT_PRIMARY },
  meditatePill: {
    backgroundColor: BUTTON_CREAM,
    paddingVertical: 8,
    paddingHorizontal: 14,
    borderRadius: 24,
    flexDirection: "row",
    alignItems: "center",
    elevation: 1,
    shadowOpacity: 0.18,
    shadowRadius: 14,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 4, width: 0 },
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
  },
  meditateText: {
    color: TEXT_PRIMARY,
    fontSize: 13,
    fontFamily: SERIF_FONT,
    marginLeft: 6,
  },

  // CONTAINER - HOME
  containerHome: {
    marginHorizontal: 10,
    flex: 1,
    paddingBottom: 90,
  },

  // CONTAINER - MATCHES
  containerMatches: {
    justifyContent: "space-between",
    flex: 1,
    paddingHorizontal: 10,
  },

  // CONTAINER - MESSAGES
  containerMessages: {
    justifyContent: "space-between",
    flex: 1,
    paddingHorizontal: 0,
  },
  matchScreen: {
    flex: 1,
    alignItems: "center",
    justifyContent: "flex-start",
    paddingHorizontal: 24,
    paddingTop: 20,
  },
  matchCard: {
    width: "100%",
    backgroundColor: "transparent",
    borderRadius: 0,
    paddingVertical: 16,
    paddingHorizontal: 8,
    alignItems: "center",
    shadowOpacity: 0,
    shadowRadius: 0,
    shadowColor: BLACK,
    shadowOffset: { height: 0, width: 0 },
  },
  matchHeartsLayer: {
    width: "100%",
    height: 70,
    position: "relative",
    marginBottom: 6,
  },
  matchHeart: {
    position: "absolute",
    backgroundColor: "rgba(246, 246, 244, 0.9)",
    padding: 8,
    borderRadius: 20,
    shadowOpacity: 0.12,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 4, width: 0 },
  },
  matchHeartLeft: {
    left: 10,
    top: 12,
  },
  matchHeartCenter: {
    position: "absolute",
    alignSelf: "center",
    top: 10,
    zIndex: 2,
  },
  matchHeartRight: {
    right: 10,
    top: 16,
  },
  matchTitle: {
    fontSize: 26,
    color: TEXT_PRIMARY,
    fontWeight: "600",
    fontFamily: SERIF_FONT,
    marginTop: -4,
  },
  matchSubtitle: {
    marginTop: 2,
    color: TEXT_SECONDARY,
    fontSize: 15,
    textAlign: "center",
    fontFamily: SERIF_FONT,
  },
  matchAvatarRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    marginTop: 6,
    marginBottom: 14,
    position: "relative",
    height: 220,
  },
  matchAvatarRing: {
    width: 280,
    height: 280,
    borderRadius: 140,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "transparent",
    borderWidth: 0,
    borderColor: "transparent",
    shadowOpacity: 0,
    shadowRadius: 0,
    shadowColor: "transparent",
    shadowOffset: { height: 0, width: 0 },
    elevation: 0,
    position: "relative",
  },
  matchAvatarRingRight: {
    marginLeft: -18,
    backgroundColor: "rgba(174, 191, 209, 0.35)",
  },
  matchAvatar: {
    width: 80,
    height: 80,
    borderRadius: 40,
    borderWidth: 0,
    borderColor: "transparent",
    zIndex: 1,
  },
  matchAvatarHalo: {
    position: "absolute",
    width: 280,
    height: 280,
    borderRadius: 140,
    top: 0,
    left: 0,
    zIndex: 0,
  },
  matchActions: {
    width: "100%",
    marginTop: 6,
    alignItems: "center",
  },
  matchPrimaryButton: {
    backgroundColor: CTA_PRIMARY,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    width: "100%",
    maxWidth: 320,
    shadowOpacity: 0.15,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  matchPrimaryButtonText: {
    color: WHITE,
    fontSize: 16,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
    letterSpacing: 0.4,
  },
  matchSecondaryButton: {
    marginTop: 10,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    backgroundColor: BUTTON_CREAM,
    paddingVertical: 13,
    borderRadius: 24,
    alignItems: "center",
    width: "100%",
    maxWidth: 320,
    shadowOpacity: 0.12,
    shadowRadius: 8,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 4, width: 0 },
    elevation: 2,
  },
  matchSecondaryButtonText: {
    color: TEXT_PRIMARY,
    fontSize: 15,
    fontWeight: "500",
    fontFamily: SERIF_FONT,
  },
  flowTop: {
    paddingTop: 50,
    paddingHorizontal: 16,
    paddingBottom: 6,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    borderBottomWidth: 1,
    borderBottomColor: "#AEBFD1",
  },
  flowTopIcon: {
    width: 36,
    height: 36,
    borderRadius: 18,
    alignItems: "center",
    justifyContent: "center",
  },
  flowTopCenter: {
    flex: 1,
    alignItems: "center",
  },
  flowSectionHeader: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 18,
    marginTop: 16,
  },
  flowSectionTitle: {
    color: TEXT_PRIMARY,
    fontSize: 16,
    fontWeight: "700",
    letterSpacing: 1,
    textTransform: "uppercase",
  },
  flowSectionCount: {
    backgroundColor: "#FBF5EA",
    borderRadius: 12,
    paddingHorizontal: 10,
    paddingVertical: 3,
    marginLeft: 10,
    borderWidth: 1,
    borderColor: "rgba(228, 183, 110, 0.24)",
  },
  flowSectionCountText: {
    color: TEXT_PRIMARY,
    fontSize: 12,
    fontWeight: "700",
  },
  matchesRow: {
    paddingHorizontal: 14,
    paddingVertical: 12,
  },
  soulmateScreen: {
    flex: 1,
    paddingTop: 52,
  },
  soulmateHeader: {
    paddingHorizontal: 16,
    paddingBottom: 8,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  },
  soulmateTitle: {
    color: TEXT_PRIMARY,
    fontSize: 18,
    fontFamily: SERIF_FONT,
    fontWeight: "600",
  },
  soulmateList: {
    paddingHorizontal: 16,
    paddingBottom: 110,
  },
  soulmateItem: {
    alignItems: "center",
    marginBottom: 22,
  },
  soulmateHaloWrap: {
    width: 280,
    height: 280,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
  },
  soulmateSparkles: {
    position: "absolute",
    width: 280,
    height: 180,
    top: 10,
    opacity: 0.9,
    zIndex: 0,
  },
  soulmateHalo: {
    position: "absolute",
    width: 280,
    height: 280,
    top: 0,
    zIndex: 1,
  },
  soulmateAvatar: {
    width: 220,
    height: 220,
    borderRadius: 110,
    zIndex: 2,
  },
  soulmateName: {
    marginTop: 6,
    color: TEXT_PRIMARY,
    fontSize: 20,
    fontFamily: SERIF_FONT,
    fontWeight: "600",
  },
  soulmateCity: {
    marginTop: 2,
    color: TEXT_SECONDARY,
    fontSize: 14,
    fontFamily: SERIF_FONT,
  },
  soulmateFooterButton: {
    position: "absolute",
    left: 20,
    right: 20,
    bottom: 22,
    backgroundColor: CTA_PRIMARY,
    borderRadius: 24,
    paddingVertical: 14,
    alignItems: "center",
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  soulmateFooterText: {
    color: WHITE,
    fontSize: 16,
    fontFamily: SERIF_FONT,
    fontWeight: "500",
  },
  matchItem: {
    alignItems: "center",
    marginRight: 14,
  },
  matchAvatarWrap: {
    position: "relative",
  },
  matchDot: {
    position: "absolute",
    right: -2,
    bottom: 6,
    width: 16,
    height: 16,
    borderRadius: 8,
    borderWidth: 2,
    borderColor: WHITE,
  },
  matchName: {
    marginTop: 8,
    color: DARK_GRAY,
    fontSize: 13,
  },
  messageRow: {
    paddingHorizontal: 18,
    paddingVertical: 10,
    borderBottomWidth: 1,
    borderBottomColor: "#AEBFD1",
  },
  chatHeader: {
    paddingTop: 50,
    paddingHorizontal: 16,
    paddingBottom: 10,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    borderBottomWidth: 1,
    borderBottomColor: "#E4B76E",
  },
  chatHeaderCenter: {
    flexDirection: "row",
    alignItems: "center",
  },
  chatAvatarWrap: {
    width: 32,
    height: 32,
    borderRadius: 16,
    backgroundColor: "#E4B76E",
    alignItems: "center",
    justifyContent: "center",
    marginRight: 8,
  },
  chatName: {
    color: DARK_GRAY,
    fontSize: 16,
  },
  chatBody: {
    flex: 1,
    paddingHorizontal: 16,
    paddingTop: 16,
  },
  chatMatchedText: {
    color: GRAY,
    fontSize: 12,
    textAlign: "center",
    marginBottom: 14,
  },
  chatBubbleLeft: {
    alignSelf: "flex-start",
    backgroundColor: "rgba(174, 191, 209, 0.4)",
    borderRadius: 20,
    borderTopLeftRadius: 4,
    paddingVertical: 12,
    paddingHorizontal: 16,
    marginBottom: 12,
    maxWidth: "75%",
  },
  chatBubbleTextLeft: {
    color: TEXT_PRIMARY,
    fontSize: 15,
    lineHeight: 22,
    fontFamily: SERIF_FONT,
  },
  chatBubbleRight: {
    alignSelf: "flex-end",
    backgroundColor: "rgba(246, 246, 244, 0.85)",
    borderRadius: 20,
    borderTopRightRadius: 4,
    paddingVertical: 12,
    paddingHorizontal: 16,
    marginBottom: 12,
    maxWidth: "75%",
  },
  chatBubbleTextRight: {
    color: TEXT_PRIMARY,
    fontSize: 15,
    lineHeight: 22,
    fontFamily: SERIF_FONT,
  },
  chatSent: {
    color: WHITE,
    fontSize: 10,
    marginTop: 4,
    opacity: 0.8,
  },
  chatInputBar: {
    borderTopWidth: 1,
    borderTopColor: "#E4B76E",
    paddingHorizontal: 16,
    paddingVertical: 10,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    backgroundColor: WHITE,
  },
  chatInputPlaceholder: {
    color: GRAY,
    fontSize: 13,
  },
  chatSend: {
    color: CTA_PRIMARY,
    fontSize: 12,
  },
  chatTools: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-around",
    paddingVertical: 8,
    backgroundColor: WHITE,
  },
  chatToolButton: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: SURFACE_BUTTON,
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
  },
  chatToolPill: {
    paddingHorizontal: 10,
    paddingVertical: 6,
    borderRadius: 12,
    backgroundColor: SURFACE_BUTTON,
  },
  chatToolText: {
    color: TEXT_PRIMARY,
    fontSize: 12,
  },
  chatHeaderNew: {
    paddingTop: 60,
    paddingBottom: 20,
    alignItems: "center",
    position: "relative",
  },
  chatBackButton: {
    position: "absolute",
    top: 60,
    left: 20,
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: "rgba(246, 246, 244, 0.7)",
    alignItems: "center",
    justifyContent: "center",
    zIndex: 10,
  },
  chatAvatarDualWrap: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    marginBottom: 12,
  },
  chatAvatarWithHalo: {
    width: 140,
    height: 140,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
    marginHorizontal: -20,
  },
  chatHaloLeft: {
    position: "absolute",
    width: 140,
    height: 140,
    zIndex: 0,
  },
  chatHaloRight: {
    position: "absolute",
    width: 140,
    height: 140,
    zIndex: 0,
  },
  chatAvatarLeft: {
    width: 110,
    height: 110,
    borderRadius: 55,
    borderWidth: 3,
    borderColor: "rgba(246, 246, 244, 0.9)",
    zIndex: 1,
  },
  chatAvatarRight: {
    width: 110,
    height: 110,
    borderRadius: 55,
    borderWidth: 3,
    borderColor: "rgba(246, 246, 244, 0.9)",
    zIndex: 1,
  },
  chatHeaderTitle: {
    fontSize: 20,
    color: TEXT_SECONDARY,
    fontFamily: SERIF_FONT,
    textAlign: "center",
  },
  chatInfoButton: {
    position: "absolute",
    top: 60,
    right: 20,
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: "rgba(246, 246, 244, 0.7)",
    alignItems: "center",
    justifyContent: "center",
  },
  chatBodyNew: {
    flex: 1,
    paddingHorizontal: 20,
  },
  chatBodyContent: {
    paddingTop: 20,
    paddingBottom: 30,
  },
  chatInputBarNew: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 16,
    paddingVertical: 12,
    backgroundColor: "rgba(246, 246, 244, 0.85)",
    borderTopWidth: 1,
    borderTopColor: "rgba(174, 191, 209, 0.4)",
  },
  chatInputNew: {
    flex: 1,
    backgroundColor: "rgba(246, 246, 244, 0.8)",
    borderRadius: 24,
    paddingHorizontal: 16,
    paddingVertical: 10,
    fontSize: 15,
    color: TEXT_PRIMARY,
    fontFamily: SERIF_FONT,
    maxHeight: 100,
  },
  chatSendButton: {
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: PRIMARY_COLOR,
    alignItems: "center",
    justifyContent: "center",
    marginLeft: 10,
    shadowOpacity: 0.15,
    shadowRadius: 8,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 2, width: 0 },
  },

  // CONTAINER - PROFILE
  containerProfile: { marginHorizontal: 0 },
  profileCalmContent: {
    paddingHorizontal: 24,
    paddingTop: 56,
    paddingBottom: 48,
  },
  profileAvatarWrap: {
    width: 86,
    height: 86,
    borderRadius: 43,
    backgroundColor: "#F6F6F4",
    alignItems: "center",
    justifyContent: "center",
    marginBottom: 16,
  },
  profileAvatar: {
    width: 76,
    height: 76,
    borderRadius: 38,
  },
  profileHeader: {
    marginBottom: 18,
  },
  profileName: {
    fontSize: 30,
    color: DARK_GRAY,
    fontWeight: "600",
    letterSpacing: 0.2,
  },
  profileEssence: {
    marginTop: 10,
    fontSize: 15,
    lineHeight: 22,
    color: "#6E6E6E",
  },
  profileEssenceMuted: {
    marginTop: 6,
    fontSize: 14,
    lineHeight: 21,
    color: "#6E6E6E",
  },
  profileValueRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    marginTop: 6,
    marginBottom: 18,
  },
  profileValuePill: {
    backgroundColor: SURFACE_BUTTON,
    paddingHorizontal: 14,
    paddingVertical: 8,
    borderRadius: 16,
    marginRight: 10,
    marginBottom: 10,
  },
  profileValueText: {
    color: TEXT_PRIMARY,
    fontSize: 13,
    fontWeight: "600",
  },
  profilePracticeSection: {
    marginBottom: 26,
  },
  profileSectionLabel: {
    color: "#6E6E6E",
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginBottom: 10,
  },
  profileInfoRow: {
    flexDirection: "row",
    flexWrap: "wrap",
  },
  profileInfoPill: {
    backgroundColor: SURFACE_BUTTON,
    paddingHorizontal: 12,
    paddingVertical: 6,
    borderRadius: 14,
    marginRight: 8,
    marginBottom: 8,
  },
  profileInfoText: {
    color: TEXT_SECONDARY,
    fontSize: 12,
  },
  profilePrimaryCta: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: CTA_PRIMARY,
    borderRadius: 24,
    paddingVertical: 12,
    marginBottom: 22,
    shadowOpacity: 0.16,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 5, width: 0 },
  },
  profilePrimaryCtaText: {
    color: WHITE,
    fontSize: 14,
    fontWeight: "600",
    letterSpacing: 0.2,
    marginLeft: 8,
  },
  profileSecondaryActions: {
    flexDirection: "row",
    justifyContent: "space-between",
    paddingHorizontal: 12,
  },
  profileActionButton: {
    width: 52,
    height: 52,
    borderRadius: 26,
    backgroundColor: SURFACE_BUTTON,
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
  },
  containerMeditations: {
    flex: 1,
    paddingHorizontal: 10,
  },
  photo: {
    width: DIMENSION_WIDTH,
    height: 450,
  },
  topIconLeft: {
    paddingLeft: 20,
  },
  topIconRight: {
    paddingRight: 20,
  },
  actionsProfile: {
    justifyContent: "center",
    flexDirection: "row",
    alignItems: "center",
  },
  textButton: {
    fontSize: 15,
    color: WHITE,
    paddingLeft: 5,
  },
  circledButton: {
    width: 50,
    height: 50,
    borderRadius: 25,
    backgroundColor: CTA_PRIMARY,
    justifyContent: "center",
    alignItems: "center",
    marginRight: 10,
    shadowOpacity: 0.2,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 5, width: 0 },
  },
  roundedButton: {
    justifyContent: "center",
    flexDirection: "row",
    alignItems: "center",
    marginLeft: 10,
    height: 50,
    borderRadius: 24,
    backgroundColor: BUTTON_CREAM,
    paddingHorizontal: 20,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
  },
  auraHeader: {
    paddingTop: 70,
    paddingBottom: 24,
    alignItems: "center",
  },
  auraAvatarWrap: {
    width: 120,
    height: 120,
    borderRadius: 60,
    borderWidth: 4,
    borderColor: "#E4B76E",
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: WHITE,
    position: "relative",
  },
  auraAvatar: {
    width: 108,
    height: 108,
    borderRadius: 54,
    zIndex: 1,
  },
  auraAvatarHalo: {
    position: "absolute",
    width: 120,
    height: 120,
    borderRadius: 60,
    top: 0,
    left: 0,
    zIndex: 0,
  },
  auraAvatarHalo2: {
    position: "absolute",
    width: 140,
    height: 140,
    borderRadius: 70,
    top: -10,
    left: -10,
    zIndex: -1,
    opacity: 0.6,
  },
  auraProgress: {
    marginTop: 10,
    backgroundColor: PRIMARY_COLOR,
    paddingVertical: 6,
    paddingHorizontal: 18,
    borderRadius: 18,
  },
  auraProgressText: {
    color: WHITE,
    fontSize: 12,
  },
  auraNameRow: {
    flexDirection: "row",
    alignItems: "center",
    marginTop: 12,
  },
  auraName: {
    color: DARK_GRAY,
    fontSize: 24,
    marginRight: 6,
  },
  auraVerified: {
    width: 18,
    height: 18,
    borderRadius: 9,
    backgroundColor: PRIMARY_COLOR,
    alignItems: "center",
    justifyContent: "center",
  },
  auraActions: {
    flexDirection: "row",
    justifyContent: "space-around",
    paddingVertical: 18,
  },
  auraActionItem: {
    alignItems: "center",
  },
  auraActionCircle: {
    width: 56,
    height: 56,
    borderRadius: 28,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: SURFACE_BUTTON,
  },
  auraActionLabel: {
    color: TEXT_PRIMARY,
    fontSize: 12,
    marginTop: 8,
  },
  auraScreenTitle: {
    fontSize: 32,
    color: TEXT_SECONDARY,
    textAlign: "center",
    marginTop: 60,
    marginBottom: 30,
    fontFamily: SERIF_FONT,
  },
  auraProfileCard: {
    marginHorizontal: 20,
    backgroundColor: "rgba(246, 246, 244, 0.85)",
    borderRadius: 24,
    padding: 20,
    flexDirection: "row",
    alignItems: "center",
    marginBottom: 30,
    shadowOpacity: 0.1,
    shadowRadius: 12,
    shadowColor: BLACK,
    shadowOffset: { height: 4, width: 0 },
  },
  auraProfileAvatarWrap: {
    width: 80,
    height: 80,
    alignItems: "center",
    justifyContent: "center",
    position: "relative",
    marginRight: 16,
  },
  auraProfileHalo: {
    position: "absolute",
    width: 90,
    height: 90,
    opacity: 0.7,
  },
  auraProfileAvatar: {
    width: 70,
    height: 70,
    borderRadius: 35,
    borderWidth: 2,
    borderColor: "rgba(246, 246, 244, 0.9)",
  },
  auraProfileInfo: {
    flex: 1,
  },
  auraProfileName: {
    fontSize: 20,
    color: TEXT_PRIMARY,
    fontFamily: SERIF_FONT,
    fontWeight: "600",
  },
  auraProfileLocation: {
    fontSize: 15,
    color: TEXT_SECONDARY,
    marginTop: 2,
    fontFamily: SERIF_FONT,
  },
  auraEditButton: {
    backgroundColor: "rgba(246, 246, 244, 0.8)",
    paddingVertical: 8,
    paddingHorizontal: 16,
    borderRadius: 20,
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
  },
  auraEditButtonText: {
    color: TEXT_PRIMARY,
    fontSize: 14,
    fontFamily: SERIF_FONT,
  },
  auraMenuList: {
    marginHorizontal: 20,
    marginBottom: 30,
  },
  auraMenuItem: {
    backgroundColor: "rgba(246, 246, 244, 0.7)",
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 16,
    paddingHorizontal: 16,
    marginBottom: 2,
  },
  auraMenuItemLast: {
    marginBottom: 0,
  },
  auraMenuIconWrap: {
    width: 36,
    height: 36,
    alignItems: "center",
    justifyContent: "center",
    marginRight: 12,
  },
  auraMenuLabel: {
    flex: 1,
    fontSize: 16,
    color: TEXT_PRIMARY,
    fontFamily: SERIF_FONT,
  },
  auraFooter: {
    alignItems: "center",
    paddingVertical: 30,
  },
  auraFooterTitle: {
    fontSize: 18,
    color: TEXT_SECONDARY,
    fontFamily: SERIF_FONT,
    marginBottom: 4,
  },
  auraFooterVersion: {
    fontSize: 14,
    color: TEXT_SECONDARY,
    fontFamily: SERIF_FONT,
  },
  auraCard: {
    marginHorizontal: 16,
    backgroundColor: WHITE,
    borderRadius: 20,
    padding: 18,
    alignItems: "center",
    shadowOpacity: 0.08,
    shadowRadius: 12,
    shadowColor: BLACK,
    shadowOffset: { height: 6, width: 0 },
  },
  auraCardTitle: {
    color: DARK_GRAY,
    fontSize: 18,
  },
  auraCardSubtitle: {
    color: GRAY,
    fontSize: 13,
    textAlign: "center",
    marginTop: 6,
  },
  auraDots: {
    flexDirection: "row",
    marginTop: 12,
    marginBottom: 16,
  },
  auraDot: {
    width: 8,
    height: 8,
    borderRadius: 4,
    backgroundColor: "#AEBFD1",
    marginHorizontal: 4,
  },
  auraDotActive: {
    backgroundColor: CTA_PRIMARY,
  },
  auraCta: {
    backgroundColor: CTA_PRIMARY,
    borderRadius: 24,
    paddingVertical: 14,
    paddingHorizontal: 22,
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  auraCtaText: {
    color: WHITE,
    fontSize: 16,
    fontFamily: SERIF_FONT,
  },
  auraSettingsSection: {
    marginTop: 18,
    marginHorizontal: 16,
    backgroundColor: WHITE,
    borderRadius: 20,
    paddingVertical: 8,
    shadowOpacity: 0.06,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 4, width: 0 },
  },
  auraSettingRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingVertical: 12,
    paddingHorizontal: 14,
    borderBottomWidth: 1,
    borderBottomColor: "#E4B76E",
  },
  auraSettingRowLast: {
    borderBottomWidth: 0,
  },
  auraSettingLabel: {
    color: DARK_GRAY,
    fontSize: 14,
  },

  // SETTINGS
  settingsContainer: {
    flex: 1,
    paddingHorizontal: 16,
    paddingTop: 60,
  },
  settingsSection: {
    marginBottom: 18,
  },
  settingsSectionHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  },
  settingsSectionTitle: {
    color: TEXT_PRIMARY,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginBottom: 8,
  },
  settingsTag: {
    backgroundColor: CTA_PRIMARY,
    borderRadius: 12,
    paddingHorizontal: 8,
    paddingVertical: 4,
  },
  settingsTagText: {
    color: WHITE,
    fontSize: 10,
  },
  settingsHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 16,
  },
  settingsTitle: {
    color: DARK_GRAY,
    fontSize: 18,
  },
  settingsCard: {
    backgroundColor: WHITE,
    borderRadius: 20,
    paddingVertical: 8,
    shadowOpacity: 0.06,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 4, width: 0 },
  },
  settingsRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingVertical: 12,
    paddingHorizontal: 14,
    borderBottomWidth: 1,
    borderBottomColor: "#E4B76E",
  },
  settingsRowLeft: {
    flexDirection: "row",
    alignItems: "center",
  },
  settingsRowRight: {
    flexDirection: "row",
    alignItems: "center",
  },
  settingsRowLast: {
    borderBottomWidth: 0,
  },
  settingsLabel: {
    color: DARK_GRAY,
    fontSize: 14,
    marginLeft: 8,
  },
  settingsAdd: {
    color: GRAY,
    fontSize: 12,
    marginRight: 6,
  },
  preferenceCard: {
    backgroundColor: WHITE,
    borderRadius: 18,
    padding: 16,
    marginTop: 16,
  },
  preferenceText: {
    color: DARK_GRAY,
    fontSize: 14,
    marginBottom: 12,
  },
  preferenceEmphasis: {
    color: PRIMARY_COLOR,
    fontWeight: "600",
  },

  // ONBOARDING
  onboardContainer: {
    flex: 1,
    paddingHorizontal: 18,
    paddingTop: 50,
    paddingBottom: 24,
  },
  onboardHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 20,
  },
  onboardProgressTrack: {
    flex: 1,
    height: 4,
    backgroundColor: "#E4B76E",
    borderRadius: 2,
    marginHorizontal: 12,
    overflow: "hidden",
  },
  onboardProgressFill: {
    height: 4,
    backgroundColor: PRIMARY_COLOR,
  },
  onboardSkip: {
    color: PRIMARY_COLOR,
    fontSize: 12,
  },
  onboardTitle: {
    color: DARK_GRAY,
    fontSize: 24,
    marginBottom: 10,
  },
  onboardSubtitle: {
    color: GRAY,
    fontSize: 12,
    marginBottom: 12,
  },
  onboardOptions: {
    marginTop: 8,
  },
  onboardOption: {
    borderWidth: 1,
    borderColor: "#E4B76E",
    borderRadius: 20,
    paddingVertical: 12,
    paddingHorizontal: 16,
    marginBottom: 12,
    backgroundColor: WHITE,
  },
  onboardOptionActive: {
    borderColor: PRIMARY_COLOR,
    backgroundColor: "#E4B76E",
  },
  onboardOptionText: {
    color: DARK_GRAY,
    fontSize: 14,
    textAlign: "center",
  },
  onboardOptionTextActive: {
    color: PRIMARY_COLOR,
  },
  onboardList: {
    marginTop: 8,
  },
  onboardListItem: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingVertical: 12,
    borderBottomWidth: 1,
    borderBottomColor: "#E4B76E",
  },
  onboardListText: {
    color: DARK_GRAY,
    fontSize: 14,
  },
  onboardCheck: {
    width: 18,
    height: 18,
    borderRadius: 9,
    borderWidth: 1,
    borderColor: "#E4B76E",
  },
  onboardCheckActive: {
    backgroundColor: CTA_PRIMARY,
    borderColor: CTA_PRIMARY,
  },
  onboardFooter: {
    marginTop: "auto",
    marginBottom: 16,
  },
  onboardNext: {
    backgroundColor: CTA_PRIMARY,
    borderRadius: 24,
    paddingVertical: 16,
    alignItems: "center",
    shadowOpacity: 0.16,
    shadowRadius: 10,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 5, width: 0 },
    elevation: 2,
  },
  onboardNextDisabled: {
    backgroundColor: "rgba(228, 183, 110, 0.72)",
  },
  onboardNextText: {
    color: WHITE,
    fontSize: 16,
    fontFamily: SERIF_FONT,
  },
  meditationList: {
    paddingBottom: 110,
  },
  meditationCard: {
    backgroundColor: WHITE,
    borderRadius: 16,
    paddingVertical: 14,
    paddingHorizontal: 16,
    marginBottom: 12,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    elevation: 1,
    shadowOpacity: 0.08,
    shadowRadius: 10,
    shadowColor: BLACK,
    shadowOffset: { height: 6, width: 0 },
  },
  meditationTitle: {
    color: DARK_GRAY,
    fontSize: 16,
  },
  meditationMeta: {
    color: GRAY,
    fontSize: 12,
    marginTop: 4,
  },
  meditationPlay: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: SURFACE_BUTTON,
    alignItems: "center",
    justifyContent: "center",
  },
  meditationBottomBar: {
    position: "absolute",
    left: 16,
    right: 16,
    bottom: 18,
    height: 62,
    borderRadius: 20,
    backgroundColor: WHITE,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingHorizontal: 12,
    shadowOpacity: 0.18,
    shadowRadius: 16,
    shadowColor: BLACK,
    shadowOffset: { height: 8, width: 0 },
  },
  meditationPlayerDock: {
    position: "absolute",
    left: 16,
    right: 16,
    bottom: 18,
  },
  meditationBarButton: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 8,
    paddingVertical: 6,
    borderRadius: 14,
  },
  meditationBarText: {
    color: DARK_GRAY,
    fontSize: 12,
    marginLeft: 6,
  },
  meditationBarPrimary: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: CTA_PRIMARY,
    paddingHorizontal: 16,
    paddingVertical: 10,
    borderRadius: 18,
  },
  meditationBarPrimaryText: {
    color: WHITE,
    fontSize: 13,
    marginLeft: 6,
  },
  meditationPlayer: {
    backgroundColor: WHITE,
    borderRadius: 22,
    padding: 16,
    shadowOpacity: 0.18,
    shadowRadius: 16,
    shadowColor: BLACK,
    shadowOffset: { height: 8, width: 0 },
  },
  meditationPlayerHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
  },
  meditationPlayerInfo: {
    flex: 1,
    paddingRight: 12,
  },
  meditationPlayerLabel: {
    color: GRAY,
    fontSize: 11,
    textTransform: "uppercase",
    letterSpacing: 1,
  },
  meditationPlayerTitle: {
    color: DARK_GRAY,
    fontSize: 16,
    marginTop: 6,
  },
  meditationPlayerMeta: {
    color: GRAY,
    fontSize: 12,
    marginTop: 4,
  },
  meditationPlayerOpen: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: PRIMARY_COLOR,
    paddingHorizontal: 12,
    paddingVertical: 8,
    borderRadius: 16,
  },
  meditationPlayerOpenText: {
    color: WHITE,
    fontSize: 12,
    marginLeft: 6,
  },
  meditationPlayerProgress: {
    marginTop: 12,
  },
  meditationPlayerTrack: {
    height: 6,
    borderRadius: 999,
    backgroundColor: "#F6F6F4",
    overflow: "hidden",
  },
  meditationPlayerFill: {
    height: 6,
    borderRadius: 999,
    backgroundColor: PRIMARY_COLOR,
  },
  meditationPlayerControls: {
    marginTop: 16,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  },
  meditationPlayerControl: {
    width: 44,
    height: 44,
    borderRadius: 22,
    borderWidth: 1,
    borderColor: "#AEBFD1",
    alignItems: "center",
    justifyContent: "center",
    marginHorizontal: 12,
  },
  meditationPlayerMain: {
    width: 54,
    height: 54,
    borderRadius: 27,
    backgroundColor: PRIMARY_COLOR,
    alignItems: "center",
    justifyContent: "center",
  },

  // PREMIUM
  premiumContainer: {
    flex: 1,
    paddingHorizontal: 18,
    paddingTop: 60,
  },
  premiumHeader: {
    flexDirection: "row",
    alignItems: "center",
    marginBottom: 14,
  },
  premiumBrand: {
    color: PRIMARY_COLOR,
    fontSize: 18,
    marginLeft: 8,
    fontWeight: "600",
  },
  premiumTitle: {
    color: DARK_GRAY,
    fontSize: 20,
    lineHeight: 28,
  },
  premiumSectionLabel: {
    color: GRAY,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginTop: 18,
    marginBottom: 10,
  },
  premiumPlans: {
    flexDirection: "row",
    justifyContent: "space-between",
  },
  premiumPlanCard: {
    width: "48%",
    backgroundColor: WHITE,
    borderRadius: 16,
    padding: 14,
    borderWidth: 1,
    borderColor: "#E4B76E",
  },
  premiumPlanActive: {
    borderColor: PRIMARY_COLOR,
  },
  premiumPlanTitle: {
    color: DARK_GRAY,
    fontSize: 16,
  },
  premiumPlanPrice: {
    color: GRAY,
    fontSize: 12,
    marginTop: 6,
  },
  premiumPlanCheck: {
    position: "absolute",
    top: 10,
    right: 10,
    width: 20,
    height: 20,
    borderRadius: 10,
    backgroundColor: "#E4B76E",
    alignItems: "center",
    justifyContent: "center",
  },
  premiumPlanBadge: {
    position: "absolute",
    top: 10,
    right: 10,
    backgroundColor: PRIMARY_COLOR,
    borderRadius: 10,
    paddingHorizontal: 8,
    paddingVertical: 2,
  },
  premiumPlanBadgeText: {
    color: WHITE,
    fontSize: 10,
  },
  premiumList: {
    backgroundColor: WHITE,
    borderRadius: 16,
    padding: 14,
    marginTop: 16,
  },
  premiumListItem: {
    color: DARK_GRAY,
    fontSize: 13,
    marginBottom: 8,
  },
  premiumCta: {
    marginTop: 18,
    backgroundColor: CTA_PRIMARY,
    paddingVertical: 14,
    borderRadius: 24,
    alignItems: "center",
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  premiumCtaText: {
    color: WHITE,
    fontSize: 16,
    fontFamily: SERIF_FONT,
  },

  // EDIT PROFILE
  editContainer: {
    flex: 1,
    paddingHorizontal: 16,
  },
  editSection: {
    marginTop: 18,
  },
  editSectionTitle: {
    color: PRIMARY_COLOR,
    fontSize: 12,
    textTransform: "uppercase",
    letterSpacing: 1,
    marginBottom: 10,
  },
  mediaGrid: {
    flexDirection: "row",
    flexWrap: "wrap",
    justifyContent: "space-between",
  },
  mediaSlot: {
    width: "31%",
    aspectRatio: 3 / 4,
    borderRadius: 12,
    marginBottom: 12,
    overflow: "hidden",
    backgroundColor: "rgba(246, 246, 244, 0.6)",
  },
  mediaImage: {
    width: "100%",
    height: "100%",
  },
  mediaPlaceholder: {
    flex: 1,
    borderWidth: 2,
    borderStyle: "dashed",
    borderColor: "rgba(216, 140, 122, 0.4)",
    alignItems: "center",
    justifyContent: "center",
    borderRadius: 12,
    backgroundColor: "rgba(246, 246, 244, 0.5)",
  },
  mediaAdd: {
    width: 28,
    height: 28,
    borderRadius: 14,
    backgroundColor: PRIMARY_COLOR,
    alignItems: "center",
    justifyContent: "center",
  },
  mediaRemove: {
    position: "absolute",
    right: 6,
    bottom: 6,
    width: 26,
    height: 26,
    borderRadius: 13,
    backgroundColor: "rgba(43, 43, 43, 0.6)",
    alignItems: "center",
    justifyContent: "center",
  },
  editPrimaryButton: {
    marginTop: 6,
    backgroundColor: CTA_PRIMARY,
    borderRadius: 24,
    paddingVertical: 14,
    alignItems: "center",
    shadowOpacity: 0.18,
    shadowRadius: 12,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 6, width: 0 },
    elevation: 3,
  },
  editPrimaryText: {
    color: WHITE,
    fontSize: 16,
    fontFamily: SERIF_FONT,
  },
  editRow: {
    backgroundColor: WHITE,
    borderRadius: 14,
    paddingVertical: 12,
    paddingHorizontal: 14,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    marginBottom: 10,
  },
  editRowTitle: {
    color: DARK_GRAY,
    fontSize: 14,
  },
  editRowMeta: {
    color: GRAY,
    fontSize: 12,
    marginTop: 3,
  },
  editRowAction: {
    width: 28,
    height: 28,
    borderRadius: 14,
    backgroundColor: SURFACE_BUTTON,
    alignItems: "center",
    justifyContent: "center",
  },
  editOutlineButton: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: GLOW_HIGHLIGHT,
    backgroundColor: WHITE,
    borderRadius: 24,
    paddingVertical: 12,
    shadowOpacity: 0.12,
    shadowRadius: 8,
    shadowColor: BLACK,
    shadowOffset: { height: 4, width: 0 },
    elevation: 2,
  },
  editOutlineText: {
    color: TEXT_PRIMARY,
    fontSize: 15,
    fontFamily: SERIF_FONT,
    marginLeft: 6,
  },

  // MENU
  tabButtonText: {
    letterSpacing: 0.2,
    fontSize: 10,
    textAlign: "center",
    marginTop: 8,
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  tabButtonTextActive: {
    color: "#243D8E",
  },
  tabButtonTextInactive: {
    color: "#243D8E",
    opacity: 0.9,
  },
  iconMenu: {
    alignItems: "center",
  },
  tabBarWrapper: {
    position: "absolute",
    left: 0,
    right: 0,
    bottom: 16,
    alignItems: "center",
  },
  tabBarContainer: {
    width: "92%",
    height: 108,
    backgroundColor: "#F7D2C9",
    borderRadius: 28,
    flexDirection: "row",
    alignItems: "flex-start",
    justifyContent: "space-around",
    shadowOpacity: 0.16,
    shadowRadius: 22,
    shadowColor: "#8B97CF",
    shadowOffset: { height: 10, width: 0 },
    borderWidth: 1,
    borderColor: "rgba(255,255,255,0.8)",
    paddingHorizontal: 10,
    paddingTop: 10,
    paddingBottom: 24,
  },
  tabItem: {
    flex: 1,
    alignItems: "center",
    justifyContent: "flex-start",
    height: "100%",
  },
  tabBump: {
    display: "none",
    position: "absolute",
    top: -14,
    width: 60,
    height: 82,
    borderTopLeftRadius: 30,
    borderTopRightRadius: 30,
    backgroundColor: "#F6F6F4",
  },
  tabCircle: {
    width: 52,
    height: 52,
    borderRadius: 26,
    backgroundColor: "rgba(255, 239, 237, 0.92)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 2,
    borderColor: "rgba(255,255,255,0.85)",
  },
  tabCircleInactive: {
    borderWidth: 0,
    backgroundColor: "rgba(255, 239, 237, 0.72)",
  },
  tabBarHandle: {
    position: "absolute",
    bottom: 10,
    alignSelf: "center",
    width: 160,
    height: 8,
    borderRadius: 999,
    backgroundColor: "rgba(255,255,255,0.9)",
  },
  // EVENTS SCREEN
  eventsContainer: {
    flex: 1,
    paddingTop: 60,
    paddingHorizontal: 20,
  },
  eventsTitle: {
    fontSize: 32,
    fontWeight: "600",
    color: TEXT_PRIMARY,
    textAlign: "center",
    marginBottom: 20,
    fontFamily: SERIF_FONT,
  },
  eventsSearchBar: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: "rgba(246, 246, 244, 0.7)",
    borderRadius: 24,
    paddingHorizontal: 16,
    paddingVertical: 12,
    marginBottom: 20,
    borderWidth: 1,
    borderColor: "rgba(174, 191, 209, 0.5)",
  },
  eventsSearchInput: {
    flex: 1,
    marginLeft: 10,
    fontSize: 16,
    color: TEXT_PRIMARY,
  },
  eventsListContent: {
    paddingBottom: 100,
  },
  eventCard: {
    backgroundColor: "rgba(246, 246, 244, 0.85)",
    borderRadius: 20,
    marginBottom: 20,
    overflow: "hidden",
    borderWidth: 1,
    borderColor: "rgba(174, 191, 209, 0.3)",
  },
  eventCardImage: {
    width: "100%",
    height: 180,
  },
  eventCardContent: {
    padding: 16,
  },
  eventCardHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "flex-start",
    marginBottom: 6,
  },
  eventCardTitle: {
    fontSize: 22,
    fontWeight: "600",
    color: TEXT_PRIMARY,
    flex: 1,
    marginRight: 10,
    fontFamily: SERIF_FONT,
  },
  eventCardAttendees: {
    fontSize: 16,
    color: TEXT_SECONDARY,
    fontWeight: "500",
  },
  eventCardSubtitle: {
    fontSize: 15,
    color: TEXT_SECONDARY,
    marginBottom: 12,
  },
  eventCardFooter: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  },
  eventCardDate: {
    fontSize: 15,
    color: TEXT_SECONDARY,
  },
  eventCardButton: {
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 6,
    paddingHorizontal: 12,
    borderRadius: 24,
    backgroundColor: BUTTON_CREAM,
    borderWidth: 1,
    borderColor: BUTTON_BORDER,
    shadowOpacity: 0.12,
    shadowRadius: 8,
    shadowColor: PRIMARY_COLOR,
    shadowOffset: { height: 3, width: 0 },
  },
  eventCardButtonText: {
    fontSize: 15,
    color: TEXT_SECONDARY,
    marginRight: 4,
  },
  // EVENT DETAIL SCREEN
  eventDetailContainer: {
    flex: 1,
    backgroundColor: "#F7F4EE",
  },
  eventDetailHeader: {
    position: "absolute",
    top: 50,
    left: 0,
    right: 0,
    flexDirection: "row",
    justifyContent: "space-between",
    paddingHorizontal: 20,
    zIndex: 10,
  },
  eventDetailBackButton: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: "rgba(247, 244, 238, 0.82)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.06)",
  },
  eventDetailMenuButton: {
    width: 40,
    height: 40,
    borderRadius: 20,
    backgroundColor: "rgba(247, 244, 238, 0.82)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.06)",
  },
  eventDetailContent: {
    flex: 1,
    paddingHorizontal: 24,
    paddingTop: 28,
  },
  eventDetailAmbientGlow: {
    position: "absolute",
    top: 20,
    right: -40,
    width: 360,
    height: 360,
    borderRadius: 180,
    backgroundColor: "rgba(228, 183, 110, 0.10)",
    opacity: 0.5,
  },
  eventDetailAmbientSparkleCluster: {
    position: "absolute",
    top: 150,
    right: 48,
    width: 180,
    height: 140,
  },
  eventDetailSparkleDotLarge: {
    position: "absolute",
    top: 54,
    right: 20,
    width: 20,
    height: 20,
    borderRadius: 10,
    backgroundColor: "rgba(228, 183, 110, 0.52)",
  },
  eventDetailSparkleDotMedium: {
    position: "absolute",
    top: 0,
    left: 30,
    width: 14,
    height: 14,
    borderRadius: 7,
    backgroundColor: "rgba(228, 183, 110, 0.62)",
  },
  eventDetailSparkleDotSmall: {
    position: "absolute",
    bottom: 18,
    right: 78,
    width: 10,
    height: 10,
    borderRadius: 5,
    backgroundColor: "rgba(228, 183, 110, 0.48)",
  },
  eventDetailSparkleIcon: {
    position: "absolute",
    top: 28,
    right: 92,
  },
  eventDetailSparkleIconSmall: {
    position: "absolute",
    top: 84,
    left: 68,
  },
  eventDetailTitleBlock: {
    marginTop: 110,
    marginBottom: 26,
    alignItems: "flex-start",
    maxWidth: "72%",
  },
  eventDetailTitle: {
    fontSize: 38,
    lineHeight: 42,
    color: TEXT_PRIMARY,
    marginBottom: 8,
    fontFamily: "CormorantGaramond_700Bold",
  },
  eventDetailSubtitle: {
    fontSize: 18,
    lineHeight: 26,
    color: TEXT_SECONDARY,
    marginBottom: 12,
    fontFamily: SERIF_FONT,
  },
  eventDetailDescription: {
    fontSize: 16,
    lineHeight: 24,
    color: TEXT_SECONDARY,
  },
  eventDetailInfoCard: {
    backgroundColor: "rgba(255, 255, 255, 0.82)",
    borderRadius: 30,
    padding: 20,
    borderWidth: 1,
    borderColor: "rgba(43, 43, 43, 0.06)",
    shadowOpacity: 0.12,
    shadowRadius: 22,
    shadowColor: "#A88A55",
    shadowOffset: { height: 10, width: 0 },
    elevation: 5,
    marginBottom: 34,
  },
  eventDetailOrganizerRow: {
    flexDirection: "row",
    alignItems: "center",
    marginBottom: 18,
    paddingBottom: 18,
    borderBottomWidth: 1,
    borderBottomColor: "rgba(43, 43, 43, 0.06)",
  },
  eventDetailOrganizerAvatarWrap: {
    marginRight: 14,
  },
  eventDetailOrganizerLabelWrap: {
    flex: 1,
  },
  eventDetailOrganizerLabel: {
    color: TEXT_SECONDARY,
    fontSize: 16,
    marginBottom: 4,
    fontFamily: SERIF_FONT,
  },
  eventDetailHostSection: {
    flexDirection: "row",
    alignItems: "center",
    marginBottom: 24,
  },
  eventDetailHostAvatar: {
    width: 48,
    height: 48,
    borderRadius: 24,
  },
  eventDetailHostAvatarFallback: {
    width: 48,
    height: 48,
    borderRadius: 24,
    backgroundColor: PRIMARY_COLOR,
    alignItems: "center",
    justifyContent: "center",
  },
  eventDetailHostName: {
    fontSize: 22,
    color: TEXT_PRIMARY,
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  eventDetailInfoSection: {
    marginBottom: 0,
  },
  eventDetailInfoRow: {
    flexDirection: "row",
    alignItems: "center",
    marginBottom: 0,
    paddingVertical: 16,
    borderBottomWidth: 1,
    borderBottomColor: "rgba(43, 43, 43, 0.06)",
  },
  eventDetailInfoIconWrap: {
    width: 38,
    height: 38,
    borderRadius: 19,
    backgroundColor: "rgba(228, 183, 110, 0.12)",
    alignItems: "center",
    justifyContent: "center",
    marginRight: 12,
  },
  eventDetailInfoCopy: {
    flex: 1,
  },
  eventDetailInfoLabel: {
    fontSize: 14,
    color: TEXT_SECONDARY,
    marginTop: 4,
    fontFamily: SERIF_FONT,
  },
  eventDetailInfoText: {
    fontSize: 18,
    lineHeight: 22,
    color: TEXT_PRIMARY,
    fontFamily: "CormorantGaramond_600SemiBold",
  },
  eventDetailClosingNoteWrap: {
    paddingHorizontal: 22,
    marginBottom: 20,
  },
  eventDetailClosingNote: {
    textAlign: "center",
    color: TEXT_SECONDARY,
    fontSize: 16,
    lineHeight: 26,
    fontFamily: "CormorantGaramond_500Medium",
  },
  eventDetailJoinButton: {
    backgroundColor: CTA_PRIMARY,
    borderRadius: 28,
    paddingVertical: 18,
    alignItems: "center",
    marginBottom: 12,
    shadowOpacity: 0.22,
    shadowRadius: 14,
    shadowColor: "#B28A49",
    shadowOffset: { height: 8, width: 0 },
  },
  eventDetailJoinButtonText: {
    fontSize: 18,
    letterSpacing: 0.3,
    color: WHITE,
    fontFamily: "CormorantGaramond_700Bold",
  },
  eventDetailJoinNote: {
    fontSize: 14,
    color: TEXT_SECONDARY,
    textAlign: "center",
    marginBottom: 20,
  },
  // EVENT CHAT SCREEN
  eventChatBackground: {
    flex: 1,
  },
  eventChatHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    paddingTop: 50,
    paddingHorizontal: 16,
    paddingBottom: 12,
    backgroundColor: "rgba(246, 246, 244, 0.95)",
  },
  eventChatBackButton: {
    width: 40,
    height: 40,
    alignItems: "center",
    justifyContent: "center",
  },
  eventChatHeaderInfo: {
    flex: 1,
    flexDirection: "row",
    alignItems: "center",
    marginLeft: 8,
  },
  eventChatHeaderLogo: {
    width: 36,
    height: 36,
    borderRadius: 18,
    marginRight: 8,
  },
  eventChatHeaderTitle: {
    fontSize: 16,
    fontWeight: "600",
    color: TEXT_PRIMARY,
  },
  eventChatHeaderSubtitle: {
    fontSize: 14,
    color: TEXT_SECONDARY,
  },
  eventChatMenuButton: {
    width: 40,
    height: 40,
    alignItems: "center",
    justifyContent: "center",
  },
  eventChatParticipantsSection: {
    alignItems: "center",
    paddingVertical: 24,
    paddingHorizontal: 20,
  },
  eventChatParticipants: {
    flexDirection: "row",
    marginBottom: 16,
  },
  eventChatParticipantAvatar: {
    width: 80,
    height: 80,
    borderRadius: 40,
    borderWidth: 3,
    borderColor: WHITE,
  },
  eventChatDescription: {
    fontSize: 16,
    lineHeight: 24,
    color: TEXT_PRIMARY,
    textAlign: "center",
    marginBottom: 20,
  },
  eventChatTags: {
    flexDirection: "row",
    justifyContent: "center",
    gap: 16,
  },
  eventChatTag: {
    flexDirection: "row",
    alignItems: "center",
  },
  eventChatTagIcon: {
    fontSize: 18,
    marginRight: 6,
  },
  eventChatTagText: {
    fontSize: 15,
    color: TEXT_SECONDARY,
  },
  eventChatMessagesContainer: {
    flex: 1,
  },
  eventChatMessagesContent: {
    paddingHorizontal: 16,
    paddingTop: 16,
    paddingBottom: 16,
  },
  eventChatMessageBubble: {
    backgroundColor: "rgba(246, 246, 244, 0.9)",
    borderRadius: 20,
    padding: 16,
    marginBottom: 12,
  },
  eventChatMessageSender: {
    fontSize: 15,
    fontWeight: "600",
    color: TEXT_PRIMARY,
    marginBottom: 8,
  },
  eventChatMessageTime: {
    fontSize: 13,
    fontWeight: "400",
    color: TEXT_SECONDARY,
  },
  eventChatMessageText: {
    fontSize: 15,
    lineHeight: 22,
    color: TEXT_PRIMARY,
  },
  eventChatInputContainer: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 16,
    paddingVertical: 12,
    backgroundColor: "rgba(246, 246, 244, 0.95)",
    borderTopWidth: 1,
    borderTopColor: "rgba(174, 191, 209, 0.3)",
  },
  eventChatAttachButton: {
    width: 44,
    height: 44,
    alignItems: "center",
    justifyContent: "center",
  },
  eventChatInput: {
    flex: 1,
    backgroundColor: WHITE,
    borderRadius: 22,
    paddingHorizontal: 16,
    paddingVertical: 10,
    fontSize: 15,
    color: TEXT_PRIMARY,
    marginHorizontal: 8,
  },
  eventChatSendButton: {
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: "#AEBFD1",
    alignItems: "center",
    justifyContent: "center",
  },
});
